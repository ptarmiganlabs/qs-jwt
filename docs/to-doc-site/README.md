...
Files in this folder are the source of truth for updates to the qs-jwt documentation site. They are written, reviewed, and finalized here before being published to the official documentation.

## Purpose

This folder serves as a staging area for documentation that should eventually appear on the qs-jwt doc site `qs-jwt.ptarmiganlabs.com`. It is the single place where documentation is authored outside of the doc site's own repository.

## Audience

Files here should be written for **qs-jwt and Qlik Sense administrators** — not Node.js developers. Assume the reader:

- Is familiar with Qlik Sense and its ecosystem
- Has admin-level access to a Qlik Sense environment
- Understands what qs-jwt does and why they would use it
- May not know what an HTTP API is or how to read a JSON response body
- May be managing qs-jwt in a production environment
...
- Do not include internal implementation details (code snippets, internal variable names, file paths in the codebase) unless they are directly relevant to an administrator configuring or operating qs-jwt
...
- Files starting with `done_` have already been incorporated into the qs-jwt doc site, or their content has been verified to already exist there.
...
These files are maintained by the qs-jwt core team. Pull requests and issues are welcome.
